...
proc sql;
	create table raw.enrollee_pmc_bz6 as
	select *
	from cms.enrollee65_ndi_0008_clean a 
	inner join prcs.pmc_site_zip_bz6 b
...
/*change . to 0*/
data prcs.master_ndi_pmc_bz6; set master_cuz;
...
proc export	data=prcs.master_ndi_pmc_bz6
	outfile='/scratch/fatemehkp/projects/PM Components/data/processed/master_ndi_pmc_bz6.csv'
	dbms=csv
	replace;
run;

proc sort data= prcs.master_ndi_pmc_bz6 nodupkey out= prcs.site_pmc_ndi_bz6 (keep= Site_ID);
	by Site_ID; 
run;

proc export	data=prcs.site_pmc_ndi_bz6
	outfile='/scratch/fatemehkp/projects/PM Components/data/processed/site_pmc_ndi_bz6.csv'
	dbms=csv
	replace;
run;
