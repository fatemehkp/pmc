...
proc sql;
	create table pmc_site_zip_bz24 as 
	select *
	from prcs.pmc_site_zip_bz24 a 
	inner join prcs.site_pmc_ndi_bz6 b 
	on a.site_id=b.site_id;
quit;

proc sql;
	create table raw.enrollee_pmc_bz24 as
	select *
	from cms.enrollee65_ndi_0008_clean a 
	inner join pmc_site_zip_bz24 b
	on a.zip_code=b.zip_code and a.year=b.year and a.month=b.month
	where sex ne 'U';
quit;

data enrollee_pmc; 
	set raw.enrollee_pmc_bz24;
...
/*change . to 0*/
data prcs.master_ndi_pmc_bz24; set master_cuz;
...
proc export	data=prcs.master_ndi_pmc_bz24
	outfile='/scratch/fatemehkp/projects/PM Components/data/processed/master_ndi_pmc_bz24.csv'
	dbms=csv
	replace;
run;

proc sort data= prcs.master_ndi_pmc_bz24 nodupkey out= prcs.site_pmc_ndi_bz24 (keep= Site_ID);
	by Site_ID; 
run;

proc export	data=prcs.site_pmc_ndi_bz24
	outfile='/scratch/fatemehkp/projects/PM Components/data/processed/site_pmc_ndi_bz24.csv'
	dbms=csv
	replace;
run;
