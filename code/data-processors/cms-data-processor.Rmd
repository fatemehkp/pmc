...
load(here('data','processed','fac-residual-1yr.RDa'))
...
dt.ndi.fac.ec <- dt.master.ndi.pmc %>% 
  mutate(StrID = paste(age, sex, race)) %>% 
  select(Site.ID, Year, Month, StrID, no_enrollee:no_death_kidn) %>%  
  inner_join(dt.fac.ec.1yr) %>% 
  inner_join(dt.ses) %>% 
  rename(SiteID = Site.ID)

write.csv(dt.ndi.fac.ec, 
          file = here('data', 'processed', 'ndi-fac-ec.csv'),
          row.names = F)
```
...
dt.ndi.fac.residual <- dt.master.ndi.pmc %>% 
  mutate(StrID = paste(age, sex, race)) %>% 
  select(Site.ID, Year, Month, StrID, no_enrollee:no_enrollee:no_death_lungc,
         -no_death_uri, -no_death_ards) %>%  
  inner_join(dt.fac.residual.1yr) %>% 
  inner_join(dt.ses) %>% 
  rename(SiteID = Site.ID)

write.csv(dt.ndi.fac.residual, 
          file = here('data', 'processed', 'ndi-fac-residual.csv'),
          row.names = F)
```
...
save(dt.site.urban.ses.cat, file = here('data', 'processed', 'site-urban-ses-cat.RDa'))

dt.site.urban.ses.cat %>% 
  distinct(Site.ID) %>% 
  group_by(ses.cat) %>% 
  summarise(n = n())
...
```{r PM Apportioned Mass with Factors}
dt.mass <- dt.mass.app.1yr.fac %>% 
  rename_at(vars(Biomass: Traffic), ~paste("Yr", .x, sep = "_"))

dt.ses <- dt.ses.site.bz12 %>% 
  mutate(ses = SES.Zip.Norm) %>% 
  select(Site.ID, Year, ses)

dt.ndi.mass.fac <- dt.master.ndi.pmc %>% 
  mutate(StrID = paste(age, sex, race)) %>% 
  select(Site.ID, Year, Month, StrID, no_enrollee:no_death_kidn) %>%  
  inner_join(dt.mass) %>% 
  inner_join(dt.ses) %>% 
...
write.csv(dt.ndi.mass.fac, 
          file = here('data', 'processed', 'ndi-mass-fac.csv'),
          row.names = F)
```

```{r PM Apportioned Mass}
dt.mass <- dt.mass.app.1yr.c2 %>% 
  rename_at(vars(Biomass: Traffic), ~paste("Yr", .x, sep = "_"))
...
write.csv(dt.ndi.mass, 
          file = here('data', 'processed', 'ndi-mass.csv'),
          row.names = F)
```
...
dt.pmc5 <- dt.pmc.5yr %>% 
  select(Site.ID, Year, Month, cmp.major) %>% 
  rename_at(cmp.major, ~paste("Yr5", .x, sep = "_"))

dt.pmc4 <- dt.pmc.4yr.5yr %>% 
  select(Site.ID, Year, Month, cmp.major) %>% 
  rename_at(cmp.major, ~paste("Yr4", .x, sep = "_"))

dt.pmc3 <- dt.pmc.3yr.5yr %>% 
  select(Site.ID, Year, Month, cmp.major) %>% 
  rename_at(cmp.major, ~paste("Yr3", .x, sep = "_"))

dt.pmc2 <- dt.pmc.2yr.5yr %>% 
  select(Site.ID, Year, Month, cmp.major) %>% 
  rename_at(cmp.major, ~paste("Yr2", .x, sep = "_"))

dt.pmc1 <- dt.pmc.1yr.5yr %>% 
  select(Site.ID, Year, Month, cmp.major) %>% 
  rename_at(cmp.major, ~paste("Yr1", .x, sep = "_"))

dt.pmc.major <- dt.pmc1 %>% 
  merge(dt.pmc2) %>% merge(dt.pmc3) %>% 
  merge(dt.pmc4) %>% merge(dt.pmc5)
...
dt.ndi.pmc.bz6.15yr <- dt.master.ndi.pmc.bz6 %>% 
  mutate(StrID = paste(age, sex, race)) %>% 
  select(Site.ID, Year, Month, StrID, no_enrollee:no_death_lungc) %>% 
  inner_join(dt.pmc.major) %>% 
  inner_join(dt.ses) %>% 
  rename(SiteID = Site.ID)

write.csv(dt.ndi.pmc.bz6.15yr, 
          file = here('data', 'processed', 'ndi-pmc-bz6-15yr.csv'),
          row.names = F)
```
...
dt.ndi.pmc.bz12.15yr <- dt.master.ndi.pmc.bz12 %>% 
  mutate(StrID = paste(age, sex, race)) %>% 
  select(Site.ID, Year, Month, StrID, no_enrollee:no_death_lungc) %>% 
  inner_join(dt.pmc.major) %>% 
  inner_join(dt.ses) %>% 
...
write.csv(dt.ndi.pmc.bz12.15yr, 
          file = here('data', 'processed', 'ndi-pmc-bz12-15yr.csv'),
          row.names = F)
```
...
dt.ndi.pmc.bz24.15yr <- dt.master.ndi.pmc.bz24 %>% 
  mutate(StrID = paste(age, sex, race)) %>% 
  select(Site.ID, Year, Month, StrID, no_enrollee:no_death_lungc) %>% 
  inner_join(dt.pmc.major) %>% 
  inner_join(dt.ses) %>% 
...
write.csv(dt.ndi.pmc.bz24.15yr, 
          file = here('data', 'processed', 'ndi-pmc-bz24-15yr.csv'),
          row.names = F)
```
