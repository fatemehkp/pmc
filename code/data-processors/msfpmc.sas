...
proc sql;
	create table raw.enrollee_pmc as
	select *
...
/*change . to 0*/
data prcs.master_ndi_pmc; set master_cuz;
...
proc export	data=prcs.master_ndi_pmc
	outfile='/scratch/fatemehkp/projects/PM Components/data/processed/master_ndi_pmc.csv'
	dbms=csv
	replace;
run;

proc sort data= prcs.master_ndi_pmc nodupkey out= prcs.site_pmc_ndi (keep= Site_ID);
	by Site_ID; 
run;

proc export	data=prcs.site_pmc_ndi
	outfile='/scratch/fatemehkp/projects/PM Components/data/processed/site_pmc_ndi.csv'
	dbms=csv
	replace;
run;
