...
# pick the site with shorter distance
site.dist.shorter <- dt.comp %>% 
  group_by(Zip.Code, Year) %>% 
  filter(Dist == min (Dist)) %>% 
  arrange(Zip.Code, Year)

dt.comp.site.zip.bz6 <- dt.comp %>% 
  filter(Dist <= 6)
dt.comp.site.zip.bz6 %>% distinct(Zip.Code) %>% nrow() #4351
dt.comp.site.zip.bz6 %>% distinct(Site.ID) %>% nrow() #286

dt.comp.site.zip.bz12  <- dt.comp %>% 
  filter(Dist <= 12)
dt.comp.site.zip.bz12 %>% distinct(Zip.Code) %>% nrow() #8454
dt.comp.site.zip.bz12 %>% distinct(Site.ID) %>% nrow() #331

dt.comp.site.zip.bz24  <- dt.comp %>% 
  filter(Dist <= 24)
dt.comp.site.zip.bz24 %>% distinct(Zip.Code) %>% nrow() #15856
dt.comp.site.zip.bz24 %>% distinct(Site.ID) %>% nrow() #344
```

```{r}
save(dt.comp.cms, file = here('data', 'processed', 'comp-cms.RDa'))
