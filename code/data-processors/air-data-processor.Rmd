...
### This program:
  ...
  (9) Uses cross-walk files bw Sites and Zipcodes to assign the measurements from
      Sites to the Zipcodes in their x-mile bufferzone
...
# For both EC & OC the sites with both methods (TOR & TOT) are in Urban and Sub...
# ...Urban area > We use the TOT since most of the Urbans use TOT
```
...
site.spec <- dt.spec.w %>% distinct(Site.ID)
site.imprv <- dt.imprv.raw %>% distinct(Site.ID) 
...
```{r add new component data to epa data}
...
monitor.pmc <- dt.pmc.w %>% distinct(Monitor.ID) #691
site.pmc <- dt.pmc.w %>% distinct(Site.ID) #574
```
...
load('C:\\Users\\fkazem01\\Box\\Projects\\USA Spatial\\data\\processed\\site-zip-closest-bz12.RDa')

# closest zipcode to each site in each year in 12-mile bufferzone
dt.site.close.zip.pmc <- site.pmc %>% 
  merge(dt.site.zip.closest.bz12) %>% 
  arrange(Site.ID, Year)

save(dt.site.close.zip.pmc, file = here('data', 'processed', 'site-close-zip-pmc.RDa'))
write.csv(dt.site.close.zip.pmc, file = here('data', 'processed', 'site-close-zip-pmc.csv'))

dt.site.close.zip.pmc %>% distinct(Site.ID) %>% nrow() #560
# 560 out of 574 sites have zipcodes in their 12-mile buffer
...
dt.pmc.c <- dt.pmc.pm.w[complete.cases(dt.pmc.pm.w),] 
monitor.pmc <- dt.pmc.c %>% distinct(Monitor.ID) # 698 -> 530  -> 515
site.pmc <- dt.pmc.c %>% distinct(Site.ID) #574 -> 486 -> 471

# Find sites with at least 4day, 9month and 4 year
# Using Air_Daily_Process function
...
monitor.pmc <- dt.pmc.sel %>% distinct(Monitor.ID) # 698 -> 530 -> 346 -> 318
site.pmc <- dt.pmc.sel %>% distinct(Site.ID) #574 -> 486 -> 346 -> 318
...
dt.site.pmc <- dt.pmc.1yr %>% distinct(Site.ID) %>%  #574 -> 486 -> 346 -> 318
  merge(dt.aqs.sites)
...
# It has 328 sites with 8454 zipcodes around them
```
