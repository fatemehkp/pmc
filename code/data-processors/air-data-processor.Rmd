...
dt.test <- Air.Daily.Process(dt = dt.spec.raw, d = 4, m = 9, y = 4)
...
dt.imprv.raw.l <- dt.imprv.raw %>% 
  pivot_longer(cols = AS:ZN,
               names_to = "Parameter.Symbol",
               values_to = "Arithmetic.Mean",
               values_drop_na = T)

dt.test3 <- Air.Daily.Process(dt = dt.imprv.raw.l, d = 4, m = 9, y = 4)
...
dt.site.close.zip.pmc %>% distinct(Site.ID) %>% nrow() #574 > 560
...
dt.pmc.pm.w %>% distinct(Site.ID) %>% nrow() # 574 > 560 > 558
...
dt.pmc.c <- dt.pmc.pm.w[complete.cases(dt.pmc.pm.w),] #241,624
...
monitor.pmc <- dt.pmc.sel %>% distinct(Monitor.ID) # 691 > 673 > 515 > 331
site.pmc <- dt.pmc.sel %>% distinct(Site.ID) #574 -> 558 > 471 > 318
...
monitor.pmc <- dt.pmc.sel %>% distinct(Monitor.ID) # 691 > 673 > 515 > 331 > 318
site.pmc <- dt.pmc.sel %>% distinct(Site.ID) #574 -> 558 > 471 > 318 > 318
