...
site.diff <- setdiff(site.imprv$Site.ID, site.spec$Site.ID) %>% 
  as.data.frame()
colnames(site.diff) <- 'Site.ID'
...
dt.site.carbon.method <- site.diff %>% 
...
map(dt.pmc.w, ~sum(is.na(.)))

#sites with measurements for all components - spec & improve
dt.pmc.c <- dt.pmc.w[complete.cases(dt.pmc.w),] 
monitor.pmc <- dt.pmc.c %>% distinct(Monitor.ID) # 698 -> 530 
site.pmc <- dt.pmc.c %>% distinct(Site.ID) #574 -> 486
...
monitor.pmc <- dt.pmc.sel %>% distinct(Monitor.ID) # 698 -> 530 -> 346
site.pmc <- dt.pmc.sel %>% distinct(Site.ID) #574 -> 486 -> 346
...
dt.site.pmc <- dt.pmc.1yr %>% distinct(Site.ID) %>%  #574 -> 486 -> 346 -> 346
  merge(dt.aqs.sites)
...
dt.pmc.site.zip.bz6 %>% distinct(Zip.Code) %>% nrow() #4351
dt.pmc.site.zip.bz6 %>% distinct(Site.ID) %>% nrow() #346 > 277
...
# Count number of zipcodes and sites
dt.pmc.site.zip.bz12 %>% distinct(Zip.Code) %>% nrow() #8454
dt.pmc.site.zip.bz12 %>% distinct(Site.ID) %>% nrow() #346 > 328
...
# Count number of zipcodes and sites
dt.pmc.site.zip.bz24 %>% distinct(Zip.Code) %>% nrow() #15856
dt.pmc.site.zip.bz24 %>% distinct(Site.ID) %>% nrow() #346 > 341
