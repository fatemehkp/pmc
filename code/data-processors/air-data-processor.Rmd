...
dt.site.comp <- dt.comp.1yr %>% distinct(Site.ID) %>%  #574 -> 486 -> 346 -> 346
  merge(dt.aqs.sites)
save(dt.site.comp, file = here('data', 'processed', 'site-comp.RDa'))
...
load(here('data', 'processed', 'comp-1yr.RDa'))
...
# pick the site with shorter distance to zipcode centroeid
dt.site.dist.shorter <- dt.comp %>% 
...
dt.comp.site.zip.bz6 <- dt.site.dist.shorter %>% 
  filter(Dist <= 6) %>% 
  select(Zip.Code, Year, Month, Site.ID, psuedo)

save(dt.comp.site.zip.bz6, file = here('data', 'processed', 'comp-site-zip-bz6.RDa'))
write.csv(dt.comp.site.zip.bz6, file = here('data', 'processed', 'comp-site-zip-bz6.csv'))

# Count number of zipcodes and sites
dt.comp.site.zip.bz6 %>% distinct(Zip.Code) %>% nrow() #4351
dt.comp.site.zip.bz6 %>% distinct(Site.ID) %>% nrow() #277

# 12-mile bufferzone around monitoring site
dt.comp.site.zip.bz12  <- dt.site.dist.shorter %>% 
  filter(Dist <= 12) %>% 
  select(Zip.Code, Year, Month, Site.ID, psuedo)

save(dt.comp.site.zip.bz12, file = here('data', 'processed', 'comp-site-zip-bz12.RDa'))
write.csv(dt.comp.site.zip.bz12, file = here('data', 'processed', 'comp-site-zip-bz12.csv'))

# Count number of zipcodes and sites
dt.comp.site.zip.bz12 %>% distinct(Zip.Code) %>% nrow() #8454
dt.comp.site.zip.bz12 %>% distinct(Site.ID) %>% nrow() #328

# 24-mile bufferzone around monitoring site
dt.comp.site.zip.bz24  <- dt.site.dist.shorter %>% 
  filter(Dist <= 24) %>% 
  select(Zip.Code, Year, Month, Site.ID, psuedo)

save(dt.comp.site.zip.bz24, file = here('data', 'processed', 'comp-site-zip-bz24.RDa'))
write.csv(dt.comp.site.zip.bz24, file = here('data', 'processed', 'comp-site-zip-bz24.csv'))

# Count number of zipcodes and sites
dt.comp.site.zip.bz24 %>% distinct(Zip.Code) %>% nrow() #15856
dt.comp.site.zip.bz24 %>% distinct(Site.ID) %>% nrow() #341
```
