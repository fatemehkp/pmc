...
write.csv(dt.pmc.site.all.zip.bz12, file = here('data', 'processed', 'pmc-site-all-zip-bz12.csv'), row.names = F)
...
write.csv(dt.pmc.1yr, file = here('data', 'processed', 'pmc-1yr.csv'), row.names = F)
...
write.csv(dt.pmc.site.ndi, file = here('data', 'processed', 'pmc-site-ndi.csv'),
          row.names = F)
...
test <- dt.pmc.daily %>% 
  group_by(Site.ID) %>% 
  nest() %>% 
  mutate(model = map(data, ~lm(PM ~ AS, data = .)),
         resid = map(model, residuals)) %>% 
  unnest(data, resid)
#208037
test2 <- unlist(unlist(test$model))
test2 <- residuals(test$model[[3]])

d <- dt.pmc.daily %>% 
  filter(Site.ID == "01-073-0023")

model <- lm(data = d, PM ~AS)
