...
```{r List of Major PM Components Yr Sub}
pmc.major.yr <- c("Yr1_SI","Yr1_NO3","Yr1_SO4",
                "Yr2_SI","Yr2_NO3","Yr2_SO4",
                "Yr3_SI","Yr3_NO3","Yr3_SO4",
                "Yr4_SI","Yr4_NO3","Yr4_SO4",
                "Yr5_SI","Yr5_NO3","Yr5_SO4")
```

```{r List of Sources}
so <- c("Yr_Biomass","Yr_Coal","Yr_Metals","Yr_Oil",
         "Yr_Salt","Yr_Soil","Yr_Steel","Yr_Traffic")
```
...
```{r ses adj. model for factors - urban}
dataname <- "ndi-fac-urban.csv"
expo <- fac
subg <- "urb"
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','fac-urban', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for factors - rural}
dataname <- "ndi-fac-rural.csv"
expo <- fac
subg <- "rur"
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','fac-rural', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for factors - west}
dataname <- "ndi-fac-west.csv"
expo <- fac
subg <- "rW"
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','fac-west', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for factors - midwest}
dataname <- "ndi-fac-midwest.csv"
expo <- fac
subg <- "rMW"
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','fac-midwest', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for factors - south}
dataname <- "ndi-fac-south.csv"
expo <- fac
subg <- "rS"
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','fac-south', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for factors - northeast}
dataname <- "ndi-fac-northeast.csv"
expo <- fac
subg <- "rNE"
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','fac-northeast', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for components - bz6}
...
```{r ses adj. model for components - bz12}
...
```{r ses adj. model for components - bz24}
...
```{r ses adj. model for apportioned mass}
dataname <- "ndi-mass.csv"
expo <- so
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    z <- gsub("exposure", expo[i], y)
    for (j in seq(1,length(cuz.sub),1)){
      w <- gsub("outcome", cuz.sub[j], z)
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','mass-ses', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for apportioned mass factor}
dataname <- "ndi-mass-fac.csv"
expo <- so
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','mass-fac-ses', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for components - bz6 & 1-5yr}
dataname <- "ndi-pmc-bz6-15yr.csv"
expo <- pmc.major.yr
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','pmc-bz6-15yr', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for components - bz12 & 1-5yr}
dataname <- "ndi-pmc-bz12-15yr.csv"
expo <- pmc.sub.yr
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','pmc-bz12-15yr', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
...
```{r ses adj. model for components - bz 24 & 1-5yr}
dataname <- "ndi-pmc-bz24.csv"
expo <- pmc.sub.yr
...
for (i in seq(1,length(expo),1)){
  ...
  for( f in  filenames ){
    ...
    for (j in seq(1,length(cuz.sub),1)){
      ...
      cat(w, file=f, sep="\n")
      k=j+length(cuz.sub)*(i-1)
      writeLines(w, here('config-files','pmc-bz24-15yr', paste(k,'.txt', sep = "")))
    }
  }
  file.remove(f)
  }
