...
load(here('data','processed', 'mass-apportioned.RDa'))
```
...
```{r - Single Tracer Mass Apportionment - function}
...
```{r - Single Tracer Mass Apportionment - Combination 2}
dt.mass.app.c2 <- dt.pmc.daily %>% 
  mutate(Soil = so.coef(high.health, 'SI') * SI,
         Metal = so.coef(high.health, 'PB') * PB,
