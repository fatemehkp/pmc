...
Plot.Single(dt = data, ggtheme = ggtheme.single) +
ggsave(here('output','plot','mass-ses.png'), width = 6.5, height = 6.5)
```
...
plot.shapes <- c(0,1,15,16)
guide.shapes <- c(15,16)

Plot.Multi(dt = data, ggtheme = ggtheme.multi.1, n = 1) +
...
plot.shapes <- c(0,1,2,5,15,16,17,18)
...
plot.cols <- c("#01742F", "#FFA42E", "#EB2226")
plot.shapes <- c(0,1,2,15,16,17)
...
data <- out.pmc.expo %>% 
  ...
  mutate(Cause = factor(Cause, levels = cuz),
         ...
         sig = ifelse(HR.U < 1 , 15, #15-prtc
                      ifelse((HR.L<=1 & HR.U>=1), 0, 15)), #0-null #15-sigpos
         sig = ifelse((sig == 0 & Window == 5), 6,
                      ifelse((sig == 15 & Window == 5),25, sig)),
...
plot.shapes <- c(0,1,2,5,4,15,16,17,18,8)
